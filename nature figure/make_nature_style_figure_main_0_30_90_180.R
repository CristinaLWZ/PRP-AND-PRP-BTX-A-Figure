...
figure_stem <- file.path(output_dir, "nature_style_hair_response_main_0_30_90_180")

line_time_key <- data.frame(
...
effect_time_key <- data.frame(
  timepoint_cn = c("30天", "90天", "180天"),
  day = c(30, 90, 180),
  day_label = c("30", "90", "180"),
  stringsAsFactors = FALSE
)
...
long_df <- raw_df %>%
  ...
  left_join(line_time_key, by = "timepoint_cn") %>%
...
change_df <- raw_df %>%
  ...
  left_join(effect_time_key, by = "timepoint_cn") %>%
  mutate(
...
write.csv(
  ...
  file = file.path(output_dir, "nature_style_hair_response_main_0_30_90_180_source_data_long.csv"),
  row.names = FALSE,
  fileEncoding = "UTF-8"
)
...
write.csv(
  ...
  file = file.path(output_dir, "nature_style_hair_response_main_0_30_90_180_summary_stats.csv"),
  row.names = FALSE,
  fileEncoding = "UTF-8"
)
...
write.csv(
  ...
  file = file.path(output_dir, "nature_style_hair_response_main_0_30_90_180_effects.csv"),
  row.names = FALSE,
  fileEncoding = "UTF-8"
)
...
p_a <- ggplot() +
  ...
  scale_x_continuous(
    limits = c(0, 194),
    breaks = line_time_key$day,
    labels = line_time_key$day_label,
    expand = expansion(mult = c(0.01, 0.01))
  ) +
  labs(
    title = "Primary response in hair density",
    subtitle = "Baseline, 30, 90 and 180-day trajectories",
...
p_b <- ggplot(
  ...
  labs(
    title = "Supportive phenotypes at selected timepoints",
...
heatmap_df <- effect_df %>%
  mutate(
    ...
    day_factor = factor(day, levels = c(30, 90, 180), labels = c("30", "90", "180"))
  )
...
p_c <- ggplot(
  ...
  labs(
    title = "Selected-timepoint effect map across all endpoints",
...
contract_lines <- c(
  "Provisional core conclusion: the treatment group shows stronger improvement than control when the figure is restricted to baseline, day 30, day 90 and day 180 data.",
  ...
  "  b: supportive selected-timepoint trajectories for four secondary endpoints.",
  "  c: selected-timepoint heatmap of benefit-oriented standardized effects at 30, 90 and 180 days.",
  ...
  "Source data needed: filtered patient-level long-format data, summary statistics, and effect-size table.",
  "Reviewer risk: baseline is kept in line panels but not in the effect heatmap because change-from-baseline at baseline is structurally zero."
)

writeLines(contract_lines, con = file.path(output_dir, "nature_style_hair_response_main_0_30_90_180_contract.txt"))
