...
figure_stem <- file.path(output_dir, "nature_style_hair_response")
...
theme_set(
  theme_classic(base_size = 6.4, base_family = "Arial") +
    theme(
      ...
      strip.background = element_rect(fill = "#F4EFE8", colour = NA),
      ...
      plot.caption = element_text(size = 5.2, colour = "#4B5563", hjust = 0),
...
hedges_g_stats <- function(treatment_values, control_values) {
  ...
  stats::t.test(treatment_values, control_values) -> tt
...
long_df <- raw_df %>%
  ...
  mutate(
    ...
    group_cn = factor(group_cn, levels = c("实验组", "对照组")),
...
write.csv(
  source_data_long,
  file = file.path(output_dir, "nature_style_hair_response_source_data_long.csv"),
  row.names = FALSE,
  fileEncoding = "UTF-8"
)
write.csv(
  summary_export,
  file = file.path(output_dir, "nature_style_hair_response_summary_stats.csv"),
  row.names = FALSE,
  fileEncoding = "UTF-8"
)
write.csv(
  effect_export,
  file = file.path(output_dir, "nature_style_hair_response_effects.csv"),
  row.names = FALSE,
  fileEncoding = "UTF-8"
)
...
contract_lines <- c(
  "Core conclusion: the intervention group shows a stronger delayed improvement in hair density, aligned secondary hair-growth endpoints, and a coordinated shift in scalp microenvironment compared with control.",
...
writeLines(contract_lines, con = file.path(output_dir, "nature_style_hair_response_contract.txt"))

qa_lines <- c(
  "Export bundle:",
  "  nature_style_hair_response.svg",
  "  nature_style_hair_response.pdf",
  "  nature_style_hair_response.tiff",
  "  nature_style_hair_response_preview.png",
  "Text/font policy:",
  ...
  "Source data:",
  "  nature_style_hair_response_source_data_long.csv",
  "  nature_style_hair_response_summary_stats.csv",
  "  nature_style_hair_response_effects.csv"
)

writeLines(qa_lines, con = file.path(output_dir, "nature_style_hair_response_QA_notes.txt"))
