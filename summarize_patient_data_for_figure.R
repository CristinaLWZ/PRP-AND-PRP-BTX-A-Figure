...
cat("\nCORRELATION AT DAY180 CHANGE\n")
wide_change <- change_df %>%
  select(`患者编号`, `分组`, metric, `180天变化`) %>%
  pivot_wider(names_from = metric, values_from = `180天变化`)

if (all(c("毛发密度", "头皮血流") %in% names(wide_change))) {
  cor_res <- cor.test(wide_change$毛发密度, wide_change$头皮血流)
  print(cor_res)
}
